...
export const getAllEvents = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    next(new AppError('Failed to fetch events', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
...
export const getEventById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!Types.ObjectId.isValid(id)) {
      return next(new AppError('Invalid event ID', 400, ErrorCodes.VALIDATION_ERROR));
    }
...
export const createEvent = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { title, description, dateTime, location, capacity, visibility, communityId } = req.body;
    ...
    next(new AppError('Failed to create event', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
...
export const updateEvent = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!isCreator && !isAdmin) {
      return next(new AppError('Not authorized to update this event', 403, ErrorCodes.FORBIDDEN));
    }
    ...
    next(new AppError('Failed to update event', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
...
export const deleteEvent = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!isCreator && !isAdmin) {
      return next(new AppError('Not authorized to delete this event', 403, ErrorCodes.FORBIDDEN));
    }
    ...
    next(new AppError('Failed to delete event', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
...
export const getEventAttendees = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    next(new AppError('Failed to fetch attendees', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
  ...
  try {
    ...
    visibilityCounts.forEach(item => {
      if (item._id in eventsByVisibility) {
        eventsByVisibility[item._id as keyof typeof eventsByVisibility] = item.count;
      }
    });
    ...
    next(new AppError('Failed to fetch event metrics', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
 ...
 */
export const getEventsForChart = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    next(new AppError('Failed to fetch events chart data', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
 ...
 */
export const exportEventToCalendar = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    next(new AppError(error instanceof Error ? error.message : 'Failed to export event', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
 ...
 */
export const exportMultipleEventsToCalendar = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { ids } = req.body;
    
    if (!Array.isArray(ids) || ids.length === 0) {
      return next(new AppError('Invalid or empty event IDs', 400, ErrorCodes.VALIDATION_ERROR));
    }
    
    // Validate all IDs
    const validIds = ids.filter(id => Types.ObjectId.isValid(id));
    if (validIds.length !== ids.length) {
      return next(new AppError('One or more invalid event IDs', 400, ErrorCodes.VALIDATION_ERROR));
    }
    ...
    // Set response headers for file download
    res.setHeader('Content-Type', 'text/calendar');
    res.setHeader('Content-Disposition', `attachment; filename="events-${new Date().toISOString().slice(0, 10)}.ics"`);
    
    // Send the calendar data
    res.send(calendar);
  } catch (error) {
    next(new AppError(error instanceof Error ? error.message : 'Failed to export events', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
 ...
 */
export const getCalendarIntegrationUrls = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    next(new AppError(error instanceof Error ? error.message : 'Failed to generate calendar URLs', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
 ...
 */
export const getRecommendedEvents = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const userId = req.user?._id;
    if (!userId) {
      return next(new AppError('User not authenticated', 401, ErrorCodes.UNAUTHORIZED));
    ...
    const recommendations = await generateEventRecommendations(userId.toString(), limitNum);
    ...
    next(new AppError(error instanceof Error ? error.message : 'Failed to generate event recommendations', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
 ...
 */
export const updateEventRecommendations = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (!Array.isArray(recommendationIds)) {
      return next(new AppError('recommendationIds must be an array', 400, ErrorCodes.VALIDATION_ERROR));
    }
    
    const validIds = recommendationIds.filter(recId => Types.ObjectId.isValid(recId));
    if (validIds.length !== recommendationIds.length) {
      return next(new AppError('One or more invalid recommendation IDs', 400, ErrorCodes.VALIDATION_ERROR));
    }
    ...
    next(new AppError(error instanceof Error ? error.message : 'Failed to update event recommendations', 500, ErrorCodes.INTERNAL_ERROR));
  }
};
