...
export class CommunityService {
  ...
  async createCommunity(communityData: Partial<ICommunity>): Promise<ICommunity> {
    return this.communityRepository.create(communityData);
  }

  async getAllCommunities(options: PaginationOptions) {
    const { docs, ...metadata } = await this.communityRepository.findAllPaginated(options);
    return {
      communities: docs,
      metadata
    };
  ...
  async updateCommunity(id: string, updateData: Partial<ICommunity>): Promise<ICommunity> {
  ...
  async joinCommunity(communityId: string, userId: string, inviteCode?: string): Promise<ICommunity> {
    ...
    const updatedCommunity = await this.communityRepository.joinCommunity(communityId, userId);
    if (!updatedCommunity) {
      throw new AppError('Failed to join community', 500, 'INTERNAL_SERVER_ERROR');
    }
  ...
  async leaveCommunity(communityId: string, userId: string): Promise<void> {
    ...
    if (community.owner.toString() === userId) {
      throw new AppError('Owner cannot leave community', 400, 'BAD_REQUEST');
    }
  ...
  async getCommunityMembers(communityId: string, options: PaginationOptions) {
    const { docs, ...metadata } = await this.communityRepository.findMembersPaginated(
      communityId,
      options
    );
    return {
  ...
  async updateCommunityIcon(communityId: string, iconPath: string): Promise<ICommunity> {
    const community = await this.getCommunityById(communityId);
  ...
  async getCommunityResources(communityId: string, queryParams: any) {
    ...
    const { docs, ...metadata } = await this.resourceRepository.findWithPagination(
      query,
      {
  ...
  async addCommunityMember(communityId: string, memberData: { userId: string; roleIds: string[] }) {
    const community = await this.getCommunityById(communityId);
    ...
    if (memberData.roleIds.length) {
      const invalidRoles = memberData.roleIds.filter(
        roleId => !community.roles.includes(new Types.ObjectId(roleId))
      );
      
      if (invalidRoles.length) {
        throw new AppError('Invalid role IDs provided', 400, 'BAD_REQUEST');
      }
    }
  ...
  async removeCommunityMember(communityId: string, userId: string) {
    ...
    if (community.owner.toString() === userId) {
      throw new AppError('Cannot remove community owner', 400, 'BAD_REQUEST');
    }
