...
CommunitySchema.index({ handle: 1 }, { unique: true });
...
CommunitySchema.methods = {
  ...
  hasRole(userId: Types.ObjectId, roleId: Types.ObjectId): boolean {
    ...
    return member ? member.roleIds.some((id: Types.ObjectId) => id.equals(roleId)) : false;
  },

  async addMember(userId: Types.ObjectId, roleIds: Types.ObjectId[] = []): Promise<void> {
  ...
  async removeMember(userId: Types.ObjectId): Promise<void> {
    this.members = this.members.filter((member: IMember) => !member.userId.equals(userId));
    this.stats.membersCount = this.members.length;
    await this.save();
  },
  ...
  getInviteLink(): string | null {
    if (!this.invite.code || !this.invite.expiresAt || this.invite.expiresAt < new Date()) {
      return null;
    }
  ...
  async updateSettings(settings: Partial<ICommunity['settings']>): Promise<void> {
  ...
  async removeDiscussion(discussionId: Types.ObjectId): Promise<void> {
    this.discussions = this.discussions.filter((id: Types.ObjectId) => !id.equals(discussionId));
    this.stats.discussionsCount = this.discussions.length;
    await this.save();
  },
  ...
  async removeResource(resourceId: Types.ObjectId): Promise<void> {
    this.resources = this.resources.filter((id: Types.ObjectId) => !id.equals(resourceId));
    this.stats.resourcesCount = this.resources.length;
    await this.save();
  },
  ...
  async removeRole(roleId: Types.ObjectId): Promise<void> {
    ...
    this.members.forEach((member: IMember) => {
      member.roleIds = member.roleIds.filter((id: Types.ObjectId) => !id.equals(roleId));
    });
    await this.save();
  },

  async assignRole(userId: Types.ObjectId, roleId: Types.ObjectId): Promise<void> {
    const member: IMember | undefined = this.members.find((m: IMember) => m.userId.equals(userId));
  ...
  async removeRoleFromMember(userId: Types.ObjectId, roleId: Types.ObjectId): Promise<void> {
    const member: IMember | undefined = this.members.find((m: IMember) => m.userId.equals(userId));
    if (member) {
      member.roleIds = member.roleIds.filter((id: Types.ObjectId) => !id.equals(roleId));
      await this.save();
