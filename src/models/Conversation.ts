...
ConversationSchema.methods = {
  ...
  removeParticipant: async function (userId: Types.ObjectId): Promise<void> {
    if (this.type === "GroupDM") {
      const isLastAdmin =
        this.participants.filter((p: IParticipant) => p.isAdmin).length === 1 &&
        this.participants.find((p: IParticipant) => p.userId.equals(userId))
          ?.isAdmin;
  ...
  // Pin/Unpin message
  togglePinMessage: async function (messageId: Types.ObjectId): Promise<void> {
...
ConversationSchema.statics = {
  ...
  findActiveConversations: async function (userId: Types.ObjectId) {
    return this.find({
